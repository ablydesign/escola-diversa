	...
	switch ( intval( $options['style'] ) ) {
		...
		case 1:
			...
			if ( $paged >= 1 && !empty( $options['prev_text'] ) ) {
				...
				if ($paged == 1){
					$prev_class_names = $class_names['previouspostslink'] . " disabled";
					$out .= "<span aria-current='prev' class='{$prev_class_names}'>$prev_text</span>";
				} else {
			...
			if ( $paged <= $total_pages && !empty( $options['next_text'] ) ) {
				...
				if ($paged == $total_pages){
					$next_class_names = $class_names['nextpostslink'] . " disabled";
					$out .= "<span aria-current='prev' class='{$next_class_names}'>$next_text</span>";
				} else {
					$out .= $instance->get_single( $paged + 1, $next_text, array(
